...
func (app *Application) NewsByCategory(category string, bot *tgbotapi.BotAPI, upd *tgbotapi.Update) {
	rUri := uri + "?q=" + category
	msg := tgbotapi.NewMessage(upd.Message.Chat.ID, "Here are the latest news from "+category+" category: \n")
	...
	fmt.Println(category, " category function called")
}
